...
export const usePasswordResetRequest = () => {
  return useMutation({
    mutationFn: async (
      data: z.infer<typeof TokenRequestSchema>
...
export const useEnable2FA = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const message =
        error.response?.data?.detail?.message || "Failed to enable 2FA";
      toast.error(message);
    },
  });
};
...
export const useResend2FACode = () => {
  return useMutation({
    mutationFn: async (
      data: z.infer<typeof TwoFactorConfirmationCreateSchema>
    ): Promise<BaseResponse> => {
...
export const use2FAManagement = () => {
  ...
  return {
    ...
    error:
      enable2FA.error || disable2FA.error || verify2FA.error || resend2FA.error,
  };
};
